...
export default function Footer() {
  ...
  return (
    <div className="mt-10 h-full border-t-[1px] border-slate-300 pb-4 dark:border-darkbg dark:bg-darknav">
      <div className="my-0 mx-auto flex  max-w-[500px] justify-between py-10">
        <div>
          <ul>
            {socialMedias.map((item) => (
              <a key={item.id} href={item.link} target="_blank" rel="noopener noreferrer">
                <li className="mb-4 text-center hover:text-blue-500 dark:hover:text-blue-500 ">{item.name}</li>
              </a>
            ))}
          </ul>
        </div>
